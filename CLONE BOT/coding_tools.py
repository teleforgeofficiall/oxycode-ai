"""
Hermes-Style Coding Tools for OXYGENT
- 7-tool sandbox: web_search, read_file, write_file, search_files, patch, terminal, execute_code
    ...
    try:
        if dep_type == 'website':
            ...
            if proc.returncode != 0:
                ...
                logger.warning(f"Vercel rm failed: {err}")
                # Still remove from DB even if Vercel fails (might be already deleted)
            ...
            if proc.returncode != 0:
                err = stderr.decode("utf-8", errors="replace")[:200]
                logger.warning(f"Wrangler delete failed: {err}")
...
async def terminal(command: str, timeout: int = 30) -> Dict[str, Any]:
    ...
    # Block dangerous commands + VPS hosting commands
    blocked_commands = [
        ...
        # Dangerous patterns
        'chmod 777', 'chmod -R 777', 'curl.*|bash', 'wget.*|bash',
        'eval(', 'exec(', '__import__',
        # VPS hosting (must deploy via Vercel/Cloudflare only)
        'python server.py', 'python app.py', 'node server.js', 'node app.js',
        'pm2 start', 'pm2 run', 'docker run', 'docker-compose',
        'nohup', 'screen -S', 'tmux',
        'ss -tlnp', 'netstat -tlnp',  # Port scanning
    ]
...
async def deploy_website(
    ...
    try:
        ...
        if count >= max_sites:
            return {
                "success": False,
                "error": f"Deploy limit reached ({count}/{max_sites}). Delete an existing site first."
            }
    except Exception:
        max_sites = 5

    # Sanitize project name
    safe_name = re.sub(r'[^a-z0-9-]', '-', project_name.lower().strip())[:40]
    if not safe_name:
        safe_name = f"site-{uid}"

    # Create a temp deploy directory
    deploy_dir = tempfile.mkdtemp(prefix=f"vercel_{safe_name}_")
    try:
        # Copy all files from sandbox to deploy dir
        src = Path(source_dir)
        if src.exists():
        ...
        # Create vercel.json based on framework
        vercel_config = {}
        if framework == "static":
            vercel_config = {
                "buildCommand": build_command or None,
                "outputDirectory": output_dir or ".",
                "framework": None,
            }
            ...
            vercel_config = {
                "framework": framework,
                "buildCommand": build_command or None,
                "outputDirectory": output_dir or None,
            }
        ...
        # Write vercel.json if we have config
        if vercel_config:
            vercel_path = os.path.join(deploy_dir, "vercel.json")
        ...
        if proc.returncode != 0:
            logger.error(f"Vercel deploy failed: {stderr_str[:500]}")
        ...
        if not url:
            ...
            import re as _re
            url_match = _re.search(r'https://[^\s]+\.vercel\.app[^\s]*', stdout_str)
            if url_match:
                url = url_match.group(0)

        if url:
            # Add deployment to DB and return remaining limits
            try:
...
async def deploy_bot(
    ...
    try:
        ...
        if count >= max_workers:
            return {
                "success": False,
                "error": f"Worker limit reached ({count}/{max_workers}). Delete an existing worker first."
            }
    except Exception:
        max_workers = 5

    # Sanitize worker name
    safe_name = re.sub(r'[^a-z0-9-]', '-', bot_name.lower().strip())[:40]
    if not safe_name:
        safe_name = f"bot-{uid}"
    ...
    try:
        ...
        # Ensure there's an index.js (main entry point)
        index_path = os.path.join(deploy_dir, "index.js")
        if not os.path.exists(index_path):
            # Check for main.js, worker.js, or server.js
        ...
        wrangler_path = os.path.join(deploy_dir, "wrangler.jsonc")
        ...
        if proc.returncode != 0:
            logger.error(f"Wrangler deploy failed: {stderr_str[:500]}")
            return {"success": False, "error": f"Deploy failed: {stderr_str[:300]}"}
        ...
        for line in stdout_str.split("\n"):
            ...
            if "workers.dev" in line:
                import re as _re
                url_match = _re.search(r'https://[^\s]+\.workers\.dev[^\s]*', line)
                if url_match:
                    url = url_match.group(0)
                    break

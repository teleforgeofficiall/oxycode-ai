"""
OXYGENT BY OXYCODE - Database Layer
PostgreSQL (Neon DB) for users, channels, and admin settings
"""
...
def init_db():
    ...
    # Deployments table (tracks each hosted site/worker for delete)
    cursor.execute('''
    ...
    cursor.execute('INSERT INTO settings (key, value) VALUES (%s, %s) ON CONFLICT (key) DO NOTHING', ('global_max_sites', '5'))
    cursor.execute('INSERT INTO settings (key, value) VALUES (%s, %s) ON CONFLICT (key) DO NOTHING', ('global_max_workers', '5'))
...
def reset_all_data():
    """Full data reset: wipe ALL runtime rows from every table (users, sessions,
    channels, broadcasts, states, workers, payments) but KEEP the schema +
    default settings intact so the bot starts fresh without re-migrating.

    Safe to run while the bot is running (each statement auto-commits). The
    connection pool is re-initialized if it was closed.
    """
    conn = get_db()
    cursor = conn.cursor()
    tables = [
    ...
    # Truncate in dependency-safe order with CASCADE so FK-constrained tables
    # (code_sessions/user_states/broadcasts reference users) are all cleared.
    # RESTART WITH 0 resets any SERIAL/auto-id counters too.
    trunc_sql = "TRUNCATE TABLE " + ", ".join(tables) + " RESTART WITH 0 CASCADE"
    try:
        cursor.execute(trunc_sql)
    except Exception:
        # Fallback: DELETE in reverse-FK order if TRUNCATE is unsupported/blocked.
        conn.rollback()
        for t in tables:
            try:
    ...
    conn.commit()
    conn.close()
    print("[DB] All data reset (tables truncated, schema preserved).")
...
def get_referral_bonus():
    """Get bonus messages per referral"""
    try:
        return int(get_setting('referral_bonus', '20'))
    except (ValueError, TypeError):
        return 20
...
def add_worker(user_id, worker_name, worker_type="website", worker_url=None, status="pending"):
    """Add worker to database"""
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO workers (user_id, worker_name, worker_type, worker_url, status, updated_at)
        VALUES (%s, %s, %s, %s, %s, %s)
        ON CONFLICT (user_id) DO UPDATE SET
            ...
            updated_at = EXCLUDED.updated_at
    ''', (user_id, worker_name, worker_type, worker_url, status, datetime.now()))
    conn.commit()
    conn.close()
...
def get_worker(user_id):
    ...
    conn = get_db()
    cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cursor.execute('SELECT * FROM workers WHERE user_id = %s', (user_id,))
    worker = cursor.fetchone()
    conn.close()
    return dict(worker) if worker else None


def update_worker(user_id, **kwargs):
    ...
    for key, value in kwargs.items():
        if key in ['worker_name', 'worker_type', 'worker_url', 'status']:
    ...
    if updates:
        ...
        query = f"UPDATE workers SET {', '.join(updates)} WHERE user_id = %s"
...
def delete_worker(user_id):
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute('DELETE FROM workers WHERE user_id = %s', (user_id,))
    conn.commit()
    conn.close()
...
def get_user_worker_count(uid: int) -> int:
    """Return how many workers a user has deployed."""
    conn = get_db()
    cur = conn.cursor()
...
def add_deployment(uid: int, deploy_type: str, name: str, url: str) -> int:
    """Add a deployment record. Returns the deployment ID."""
    conn = get_db()
    cur = conn.cursor()
    cur.execute(
...
def remove_deployment(uid: int, deploy_id: int) -> bool:
    """Remove a deployment record. Returns True if found."""
    conn = get_db()
    cur = conn.cursor()
...
def get_user_deployments(uid: int, deploy_type: str = None) -> list:
    """Return list of a user's deployments."""
    conn = get_db()
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    if deploy_type:
...
def get_all_user_deploys() -> list:
    """Return all users with their deploy/worker counts for admin panel."""
    conn = get_db()
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute('''

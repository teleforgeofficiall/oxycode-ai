...
def get_chat_admin_only():
    """Check if chat access is restricted to admins only."""
    val = get_setting('chat_admin_only', 'false')
    return val == 'true'


def set_chat_admin_only(enabled):
    """Restrict chat access to admins only."""
    set_setting('chat_admin_only', 'true' if enabled else 'false')


def toggle_chat_admin_only():
    """Toggle admin-only chat. Returns new state."""
    current = get_chat_admin_only()
    set_chat_admin_only(not current)
    return not current

"""
OXYGENT — Hermes-Style Memory System
=====================================

A dual-layer memory system for per-user context persistence.

Memory Layers:
    1. File-based (HermesMemory)
       - MEMORY.md: Agent's personal notes about the user
       - USER.md: User profile (preferences, style, workflow)
       - Character-based limits (4000/2000 chars)
       - Entry delimiter: § (Hermes style)

    2. Database-backed (MemoryDatabase)
       - SQLite for structured key-value storage
       - Conversation history tracking
       - Category-based organization

    3. Unified (OxygentMemory)
       - Combines both layers
       - Auto-detects personal info from messages
       - Builds system prompt context

Auto-Detection:
    The system automatically extracts and saves:
    - Names ("mera naam Rahul" → name: Rahul)
    - Favorites ("mujhe pizza pasand" → favourite: pizza)
    - Age ("I am 25 years old" → age: 25)
    - City ("main Delhi se hoon" → city: Delhi)
    - Occupation ("main developer hoon" → occupation: developer)

Usage:
    memory = get_memory(user_id)
    memory.store("preference", "dark theme", "personal")
    info = memory.get_user_info("name")
